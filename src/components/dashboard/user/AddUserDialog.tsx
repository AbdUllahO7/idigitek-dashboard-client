...
export function AddUserDialog({
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-[600px] p-0 overflow-hidden rounded-lg border shadow-lg" dir={isRTL ? 'rtl' : 'ltr'}>
        <DialogHeader className="p-6 pb-2 bg-gradient-to-r from-slate-50 to-slate-100 dark:from-slate-800 dark:to-slate-900" dir={isRTL ? 'rtl' : 'ltr'}>
          <DialogTitle className={`text-xl font-semibold flex items-center ${isRTL ? 'text-left' : 'text-right'}`} dir={isRTL ? 'rtl' : 'ltr'}>
            <div className={`bg-blue-100 dark:bg-blue-900 p-2 rounded-full ${isRTL ? 'ml-3' : 'mr-3'}`} dir={isRTL ? 'rtl' : 'ltr'}>
              <Plus className="h-5 w-5 text-blue-600 dark:text-blue-400" />
            </div>
            {t('AddUserDialog.title', 'Add New User')}
          </DialogTitle>
          <DialogDescription className={`text-slate-600 dark:text-slate-400 mt-1.5 `} >
            <div dir={isRTL ? 'ltr' : 'rtl'} className={`${isRTL ?'text-right' : 'text-left'}`}>
          ...
          <div className="grid gap-5">
            <div className="grid grid-cols-2 gap-4">
              <div className="space-y-2" dir={isRTL ? 'rtl' : 'ltr'}>
                <Label htmlFor="new-firstName" className="text-sm font-medium text-left">
            ...
            <div className="grid grid-cols-2 gap-4 mt-1" dir={isRTL ? 'rtl' : 'ltr'}>
              <div className="space-y-2 pb-5">
                <Label htmlFor="new-role" className="text-sm font-medium">
                  {t('AddUserDialog.fields.role', 'User Role')}
                </Label>
                <Select value={formData.role || "user"} onValueChange={onRoleChange} dir={isRTL ? 'rtl' : 'ltr'}>
                  <SelectTrigger
                    id="new-role"
                    className={`h-10 border-slate-300 dark:border-slate-700 focus:ring-2 focus:ring-blue-500/20 ${isRTL ? 'pr-9' : 'pl-9'}`}
                  >
                    <SelectValue placeholder={t('AddUserDialog.placeholders.role', 'Select a role')} dir={isRTL ? 'rtl' : 'ltr'}/>
                  </SelectTrigger>
              ...
              <div className="space-y-2" dir={isRTL ? 'rtl' : 'ltr'}>
                ...
                <Select value={formData.status} onValueChange={onStatusChange} dir={isRTL ? 'rtl' : 'ltr'}>
                  <SelectTrigger
                    id="new-status"
                    className={`h-10 border-slate-300 dark:border-slate-700 focus:ring-2 focus:ring-blue-500/20 ${isRTL ? 'pr-9' : 'pl-9'}`}
                  >
                  ...
                  <SelectContent className="max-h-[var(--radix-select-content-available-height)] overflow-y-auto" >
                    <SelectItem value={UserStatus.ACTIVE} className="py-2.5">
                      <div className={`flex items-center ${isRTL ? 'text-left' : 'text-right'}`}>
