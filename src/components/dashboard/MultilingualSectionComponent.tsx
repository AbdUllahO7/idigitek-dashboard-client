...
import { Edit, Plus, Languages, AlertCircle, Info, ArrowRight, CheckCircle2 } from "lucide-react"
    ...
    <Card>
      ...
      <Card className="border-l-4 border-l-primary mb-6">
        <CardHeader className="pb-2">
          ...
          <CardDescription>Follow these steps to set up your {sectionName}</CardDescription>
        </CardHeader>
        <CardContent>
          <div className="space-y-4">
            <div className="flex gap-3">
              ...
              <div className="flex items-center gap-2">
                <span className="font-medium">
                  Add a {sectionName} section by clicking on the <span className="text-primary">"Add Section"</span> button
            ...
            <div className="flex gap-3">
              ...
              <div className="flex items-center gap-2">
                <span className="font-medium">
                  Once completed, the <span className="text-primary">"Add New {sectionName}"</span> button will become active
                </span>
            ...
            <div className="flex gap-3">
              ...
              <div>
                <span className="font-medium">Click on it to add sub-sections to your {sectionName}</span>
              </div>
            </div>
            ...
            <div className="flex gap-3">
              ...
              <div>
                <span className="font-medium">If you don't want any sub-section, just leave it empty!</span>
              </div>
            </div>
          </div>
        </CardContent>
            ...
            {languages.map((lang) => (
              <TabsContent key={lang.id} value={lang.id}>
                {sectionData[fields[0]?.id]?.[lang.id] ? (
                      ...
                      <div className="flex justify-between items-start">
                        <div>
                          {fields[0] && sectionData[fields[0].id]?.[lang.id] && (
                            <p className="text-sm text-muted-foreground">{sectionData[fields[0].id][lang.id]}</p>
                          )}
                          {fields[1] && sectionData[fields[1].id]?.[lang.id] && (
                            <CardTitle className="text-xl">{sectionData[fields[1].id][lang.id]}</CardTitle>
                          )}
                        </div>
                  ...
                  <div className="text-center py-6 text-muted-foreground">No content for {lang.label} yet.</div>
                )}
              </TabsContent>
            ))}
          </Tabs>
        ) : (
          <div className="text-center py-6 text-muted-foreground">{noDataMessage}</div>
        )}
      </CardContent>
