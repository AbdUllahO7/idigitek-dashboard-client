...
const createNavItemsSchema = (languageIds, activeLanguages) => {
  const schema = {};
  
  schema.logo = z.string().optional();
...
const createNavItemsDefaultValues = (languageIds, activeLanguages) => {
  const defaultValues = {
    logo: "",
  };
...
const processAndLoadNavData = (subsectionData, form, languageIds, activeLanguages, callbacks) => {
  if (!subsectionData) {
  ...
  languageIds.forEach(langId => {
    ...
    const navItems = navItemElements
      .map(element => {
        const translation = element.translations?.find(
          t => (t.language?._id === langId || t.language === langId) && t.isActive
        );
...
const NavItemsForm = forwardRef(
  ({ languageIds, activeLanguages, onDataChange, slug, ParentSectionId }, ref) => {
    ...
    const [navItemToDelete, setNavItemToDelete] = useState(null);
    ...
    const validateNavItemCounts = useRef(() => {
      ...
      const counts = Object.entries(values)
        .filter(([key]) => key !== "logo")
        .map(([_, langNavItems]) => 
          (Array.isArray(langNavItems) ? langNavItems.length : 0)
        );
    ...
    const processNavData = useRef((subsectionData) => {
      processAndLoadNavData(
        subsectionData,
        form,
        languageIds,
        activeLanguages,
    ...
    const confirmRemoveNavItem = (langCode, index) => {
      const currentItems = form.getValues()[langCode] || [];
      ...
      try {
        if (existingSubSectionId && contentElements.length > 0) {
          ...
          if (itemElements.length > 0) {
            await Promise.all(itemElements.map(async (element) => {
              try {
            ...
            setContentElements((prev) =>
              prev.filter((element) => element.name !== `Nav Item ${itemNumber}`)
            );
    ...
    const uploadLogo = async (elementId, file) => {
      if (!file) return null;
    ...
    const handleSave = async () => {
      ...
      try {
        ...
        const existingNavItems = contentElements.filter(el => 
          el.type === "text" && el.name.startsWith("Nav Item")
        ).sort((a, b) => a.order - b.order);
        ...
        const maxNavItems = Math.max(
          ...Object.entries(allFormValues)
            .filter(([key]) => key !== "logo")
            .map(([_, langItems]) => 
              (Array.isArray(langItems) ? langItems.length : 0)
            ),
        );
    ...
    const renderNavItemsFields = (control, langCode) => {
            ...
            {languageIds.map((langId) => {
              ...
              return (
                <NavLanguageCard
                  ...
                  onRemoveItem={(index) => confirmRemoveNavItem(langCode, index)}
                />
              );
            })}
          </div>
