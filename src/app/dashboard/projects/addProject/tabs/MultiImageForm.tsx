...
const MultiImageForm = forwardRef<any, MultiImageFormProps>((props, ref) => {
  ...
  // Data fetching from API
    ...
    const processMultiImageData = useCallback((subsectionData: SubSection | null) => {
        ...
        const imageElements = subsectionData.elements?.filter(el => el.type === 'image') || 
  ...
  // Handle image upload
    const handleImageUpload = useCallback((index: number, event: React.ChangeEvent<HTMLInputElement>) => {
  ...
  // Upload image to server
    const uploadImage = useCallback(async (elementId: string, file: File) => {
        if (!file) return null;
  ...
  // Save handler
    const handleSave = useCallback(async () => {
        ...
        if (contentElements.length > 0) {
            ...
            if (images[i].file) {
                const imageUrl = await uploadImage(existingElements[i]._id, images[i].file);
                if (imageUrl) {
            ...
            if (images[i].file) {
                const imageUrl = await uploadImage(newElement.data._id, images[i].file);
                updatedImageUrls[i] = imageUrl || '';
            } else {
            ...
            if (images[i].file) {
                ...
                uploadedUrls[i] = images[i].imageUrl || '';
            }
            }
        ...
        toast({
            title: existingSubSectionId ? "Multi-image section updated successfully!" : "Multi-image section created successfully!",
  ...
  // Create form ref for parent component
    createFormRef(ref, {
        ...
        extraMethods: {
        getImageFiles: () => form.getValues().images.map(img => img.file).filter(Boolean),
        saveData: handleSave
        },
        extraData: {
                ...
                <CardContent className="p-4">
                    ...
                    <div className="flex items-center space-x-2">
                        ...
                        onClick={() => {
                            ...
                            input.onchange = (e) => handleImageUpload(index, e as unknown as React.ChangeEvent<HTMLInputElement>);
                            input.click();
                        }}
                        className="flex-grow"
                        >
                        {imagePreviews[index] ? 'Change Image' : 'Upload Image'}
                        </Button>
