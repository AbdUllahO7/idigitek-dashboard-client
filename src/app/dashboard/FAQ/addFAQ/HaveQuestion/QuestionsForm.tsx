...
import {createHaveFaqQuestionsDefaultValues, createLanguageCodeMap } from "../../../services/addService/Utils/Language-default-values";
import { createFormRef, getAvailableIcons, getSafeIconValue, getSubSectionCountsByLanguage, useForceUpdate, validateSubSectionCounts } from "../../../services/addService/Utils/Expose-form-data";
...
import { faqHaveQuestionFormProps, faqHaveQuestionFormRef, FaqHaveQuestionsFormState } from "@/src/api/types/sections/FAQ/faqSection.types";
...
const QuestionsForm = forwardRef<faqHaveQuestionFormRef, faqHaveQuestionFormProps>(
  ({ languageIds, activeLanguages, onDataChange, slug, ParentSectionId },ref) => {
    ...
    const defaultValues = createHaveFaqQuestionsDefaultValues(
      languageIds,
      activeLanguages
    );
    interface FormData {
    ...
    const updateState = useCallback(
      (newState: Partial<FaqHaveQuestionsFormState>) => {
          ...
          await Promise.all(
            remainingElements.map(async (element) => {
              ...
              if (match) {
                ...
                const newOrder = element.order - 4; // Updated to account for buttonText
        ...
        processAndLoadData(
          ...
          activeLanguages,
          {
            ...
            processElementGroup: (
              ...
              const buttonTextElement = elements.find((el) =>
                el.name.includes("ButtonText")
              );
            ...
            getDefaultValue: () => [
              { icon: "Clock", title: "", description: "", buttonText: "Read More" },
            ],
          },
          {
    ...
    const handleSave = useCallback(async () => {
      ...
      try {
        ...
        for (let i = 0; i < benefitCount; i++) {
          ...
          const buttonTextElementName = `Faq ${benefitIndex} - ButtonText`; // Fixed element name
          ...
          if (!iconElement) {
            const newElement = await createContentElement.mutateAsync({
              ...
              parent: sectionId,
              isActive: true,
              order: i * 4, // Updated to account for buttonText
          ...
          if (!titleElement) {
            const newElement = await createContentElement.mutateAsync({
              ...
              parent: sectionId,
              isActive: true,
              order: i * 4 + 1, // Updated to account for buttonText
              defaultContent: "",
            });
          ...
          if (!descElement) {
            const newElement = await createContentElement.mutateAsync({
              ...
              parent: sectionId,
              isActive: true,
              order: i * 4 + 2, // Updated to account for buttonText
              defaultContent: "",
            });
          ...
          // Create or find buttonText element
          let buttonTextElement = contentElements.find(
            (el) => el.name === buttonTextElementName
          );
          if (!buttonTextElement) {
            ...
            if (buttonTextElement) {
              updateState({
                contentElements: [...contentElements, buttonTextElement],
              });
            }
          }
    ...
    useEffect(() => {
      ...
      return () => subscription.unsubscribe();
    }, [dataLoaded, isLoadingData, form, validateFormFaqCounts]);
    ...
    return (
      <div className="space-y-6">
        <LoadingDialog
          isOpen={isSaving}
          title={existingSubSectionId ? "Updating Faqs" : "Creating Faqs"}
          ...
          {benefitCountMismatch && (
            <div className="flex items-center text-amber-500 mr-4">
              ...
              <span className="text-sm">
                Each language must have the same number of FAQs
              </span>
            </div>
          )}
          <Button
          ...
          >
            {isSaving ? (
              ...
              <>
                ...
                {existingSubSectionId ? "Update Faqs" : "Save Faqs"}
              </>
            )}
          </Button>
        </div>
